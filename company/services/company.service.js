...
export default class companyService {
    ...
    static async update(company) {
      ...
      if (!exist) throw new NotFoundError('COMPANY_DOES_NOT_EXIST');
    ...
    static async get({ id }) {
      ...
      if (!company) throw new NotFoundError('COMPANY_DOES_NOT_EXIST');
    ...
    static async delete({ id }) {
      await db.Company.remove({ _id: id, userId: this.user.id });
      return { id };
    }
}
